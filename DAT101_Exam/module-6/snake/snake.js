...
checkCollision() {
    ...
    if (!collision) {
     ...
     console.log("Cell info:", boardCellInfo.infoType, "Row:", this.boardCell.row, "Col:", this.boardCell.col);
    }
...
export class TSnake {
  ...
  update() {
    ...
    if (this.#head.update()) {
      ...
      if (this.#newBodyPart) {
        GameProps.gameBoard.getCell() //HVAAAAAAAAAAA
        //when bait is eaten a new segment is waiting. add it to body instead of moving the tail. 
        this.#body.push(this.#newBodyPart);
        this.#newBodyPart = null; //clear it so only 1 gets added
      } else {
        this.#tail.update(); // no growth pending, tail moves normally
      }
    } else if (!this.#isDead) {
      this.#isDead = true;
      return false; // Collision detected, do not continue
  ...
  grow() {
    //clones the last body segment and stores it to add, then gets added on the next update tick. 
